  ...
  return <Dialog.Root>
    <Dialog.Trigger asChild>
      <button type="button" aria-label={mode === 'platform' ? t('platformCheckInsTitle', { name: name(platforms[0].nameZh, platforms[0].nameEn) }) : t('openDailyChecklist')}
        ...
        {mode === 'platform' && !compact && <span className={`text-xs ${summary.pending ? 'text-amber-700' : 'text-emerald-700'}`}>
    ...
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed inset-x-0 bottom-0 z-50 flex max-h-[90dvh] flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:inset-x-auto sm:bottom-auto sm:left-1/2 sm:top-1/2 sm:w-[calc(100%_-_2rem)] sm:max-w-xl sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-2xl">
        <div className="border-b border-zinc-100 px-5 pb-5 pt-6 sm:px-6">
          ...
          {summary.total > 0 && <div className="mt-4">
            <div className="mb-2 flex items-center justify-between text-xs">
              ...
              <span className={summary.pending ? 'text-amber-700' : 'text-emerald-700'}>{summary.pending ? t('checklistPendingCount', { count: summary.pending }) : t('checklistComplete')}</span>
            </div>
            <div className="h-1.5 overflow-hidden rounded-full bg-zinc-100" aria-hidden="true">
              <div className="h-full rounded-full bg-emerald-500 transition-all" style={{ width: `${summary.completed / summary.total * 100}%` }} />
            </div>
          </div>}
        </div>
                ...
                <div className="divide-y divide-zinc-100">
                  {items.map((item) => {
                    ...
                    return <label key={item.id} className="flex cursor-pointer items-center gap-3 px-4 py-3.5 transition-colors hover:bg-zinc-50">
                      ...
                      <span aria-hidden="true" className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border transition-colors peer-focus-visible:ring-2 peer-focus-visible:ring-emerald-500 peer-focus-visible:ring-offset-2 ${completed ? 'border-emerald-600 bg-emerald-600 text-white' : 'border-zinc-300 bg-white'}`}>
                        {completed && <Check className="h-3.5 w-3.5" />}
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className={`block break-words text-sm ${completed ? 'text-zinc-400' : 'text-zinc-800'}`}>{name(item.nameZh, item.nameEn)}</span>
                        <span className={`mt-0.5 block text-xs ${completed ? 'text-emerald-700' : 'text-zinc-400'}`}>{completed ? t('checkedInToday') : t('notCheckedInToday')}</span>
                      </span>
                      {item.reward > 0 && <span className={`shrink-0 text-xs font-medium tabular-nums ${completed ? 'text-zinc-400' : 'text-amber-700'}`}>+{item.reward} {t('creditUnit')}</span>}
                    </label>;
                  })}
                </div>
              </section>;
        ...
        <div className="border-t border-zinc-100 px-5 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] text-xs text-zinc-400 sm:px-6">{t('checklistUndoHint')}</div>
      </Dialog.Content>
    </Dialog.Portal>
  </Dialog.Root>;
}
